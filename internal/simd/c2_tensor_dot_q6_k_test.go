...
func TestTensorDotQ6_KVsCOracle(t *testing.T) {
	...
	for _, c := range srcCandidates {
		if _, err := os.Stat(filepath.Join(c, "test_dot_q6_k_oracle.c")); err == nil {
			srcDir = c
			break
		}
	}
	...
	oracleSrc := filepath.Join(srcDir, "test_dot_q6_k_oracle.c")
	cSrc := filepath.Join(srcDir, "c2_tensor_dot_q6_k.c")
	tmpDir := t.TempDir()
	tmpBin := filepath.Join(tmpDir, "test_dot_q6_k_oracle")
	...
	weight := make([]byte, numBlocks*210)
	x := make([]float32, cols)

	for b := 0; b < numBlocks; b++ {
		blk := weight[b*210 : (b+1)*210]
		// ql: 128 bytes
		for i := 0; i < 128; i++ {
			blk[i] = byte((b*23 + i*11 + 5) & 0xFF)
		}
		// qh: 64 bytes
		for i := 0; i < 64; i++ {
			blk[128+i] = byte((b*17 + i*7 + 1) & 0xFF)
		}
		// scales: 16 bytes int8
		for i := 0; i < 16; i++ {
			blk[192+i] = byte(int8((i % 7) - 3))
		}
		// d: fp16 1.0 (0x3C00)
		blk[208] = 0x00
		blk[209] = 0x3C
	...
	for i := 0; i < cols; i++ {
		x[i] = float32((i%13)-6) * 0.1
	}

	resGo := C2_tensor_dot_q6_k(weight, x, cols)
	...
	t.Logf("PARITY VALIDATED [%s]: C2_tensor_dot_q6_k is 100%% BIT-EXACT against GCC -O2 C99 oracle (Go=%f, Oracle=%f, diff=%e)!", modeStr, resGo, expectedOracle, diff)
}
